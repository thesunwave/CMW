class FavoriteWorksController < ApplicationController
  ...
  def create
    if Favorite.create(favorited: @work, user: current_user)
      redirect_to show_work_path(current_user.username, @work), notice: 'Work has been favorited'
    else
  ...
  def destroy
    ...
    redirect_to show_work_path(current_user.username, @work), notice: 'Project is no longer in favorites'
  end
