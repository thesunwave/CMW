class User < ActiveRecord::Base
  ...
  after_create :assign_default_role, 
    :assign_default_notifications,
    :assign_default_username
  ...
  def get_settings
    ...
    # добавить страну
    settings.merge!({ country_id: (self.city.present?) ? self.city.country.id : 'null' })
  ...
  def self.valid_username?(username, instance = nil)
    unless username.blank?
      ...
      wrong_names = ['admin','administartion','administrator','help','adm','flatfm','flat.fm','flat_fm','flat-fm','flat']
...
private  
  ...
  def validate_tags
    unless tags.blank?
      # проверяем является ли конвертированная строка массивом
      if tags.is_a?(Array)
        # максимальный размер массива ссылок
        max_links_size = 15
        # проверка на размер
        if tags.size > max_links_size
          # сообщение о превышении размера массива
          errors.add(:tags, I18n.t('errors.messages.too_long'))
        end
      else
        # сообщение о неправильном значении поля
        errors.add(:tags, I18n.t('errors.messages.invalid'))
      end   # if
    end   # unless
  end   # def
